...
# n_samples <- 40 # Total number of unique samples
# n_groups <- 3 # Total number of conditions/groups to distribute samples across
# 
# 
# # 1. Create Sample IDs (retains original structure)
# # This assigns sample1...sample40 repeatedly to every cell.
# pbmc3k$sample_id <- rep(
#   x = paste0("sample", seq_len(n_samples)),
#   length.out = length(pbmc3k$sample_id)
# )
# 
# # 2. Dynamically Assign Conditions to Samples
# # This logic ensures the n_samples are split as evenly as possible among n_groups.
# 
# # Define the condition names (e.g., "condition1", "condition2", ...)
# condition_names <- paste0("condition", seq_len(n_groups))
# 
# # Calculate how many samples get assigned to each group (uses ceiling for even distribution)
# reps_per_group <- ceiling(n_samples / n_groups)
# 
# # Create a condition assignment vector that is exactly n_samples long
# sample_conditions_vector <- rep(
#   condition_names,
#   each = reps_per_group,
#   length.out = n_samples
# )
# 
# # Create a map to link sample ID strings to their condition strings
# sample_ids_vector <- paste0("sample", seq_len(n_samples))
# sample_condition_map <- data.frame(
#   sample_id = sample_ids_vector,
#   condition_id_sample = sample_conditions_vector
# )
# 
# # 3. Apply the Condition Map to the Cell-Level Data
# # Use 'match' to efficiently look up the condition for every cell based on its sample ID.
# match_indices <- match(pbmc3k$sample_id, sample_condition_map$sample_id)
# pbmc3k$condition_id <- sample_condition_map$condition_id_sample[match_indices]
```
