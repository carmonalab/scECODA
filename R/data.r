...
#'
#' @format A named \code{list} where each element corresponds to a different study
#' (e.g., \code{$Adams}). Each study element is a \code{list} containing the following:
#' \itemize{
#'    \item \code{cell_counts_highresolution}: Data frame of cell type counts at high resolution annotation (samples as rows, cell types as columns).
#'    \item \code{cell_counts_lowresolution}: Data frame of cell type counts at low resolution annotation (samples as rows, cell types as columns).
