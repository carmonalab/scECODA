...
plot_barplot <- function(ecoda_object,
  ...
  if (plot_by == "group") {
    ...
    if (!is.null(custom_sample_order)) {
      ...
      if (!is.null(label_col)) {
        # 1. Get a unique list of samples and their corresponding label_col
        sample_group_map <- plot_data %>%
          distinct(sample_id, !!sym(label_col))

        # 2. Order the samples by the group column first (using mixedsort on its
        # values) and then by sample_id (using mixedsort on its values).
