test_that("Frequency calculation is accurate", {
    ...
    counts <- t(data.frame(A = c(10, 50), B = c(90, 50)))
    freq <- calc_freq(counts)

    expect_equal(colSums(freq), c(100, 100))
...
test_that("Zero imputation handles counts and frequencies correctly", {
    ...
    counts_df <- t(data.frame(A = c(10, 0), B = c(20, 10)))
    imputed_counts <- replace_zeros(counts_df, rep_method = "counts")
    ...
    freq_df <- t(data.frame(A = c(0.5, 0), B = c(0.5, 1.0)))
    imputed_freq <- replace_zeros(freq_df, rep_method = "frac_min")
...
test_that("CLR transformation is mathematically consistent", {
    ...
    pos_data <- t(data.frame(A = c(10, 5), B = c(20, 25)))
    ...
    expect_equal(colSums(clr_res), c(0, 0), tolerance = 1e-9)
})
